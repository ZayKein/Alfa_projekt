...
SQL_GOLD_TRANSFORM = """
USE DATABASE ALFA_PROJEKT;
USE SCHEMA GOLD;

-- 1. Inkrementální nahrání Faktů o prodejích (odlehčená verze bez Revenue/Cost)
INSERT INTO ALFA_PROJEKT.GOLD.FACT_SALES (ORDER_ID, PRODUCT_ID, EMPLOYEE_ID, DATE_KEY, QUANTITY, SERVICE_TYPE, SERVICE_PRICE)
SELECT 
    ORDER_ID, 
    PRODUCT_ID, 
    EMPLOYEE_ID, 
    ...
    SERVICE_TYPE, 
    SERVICE_PRICE
FROM ALFA_PROJEKT.RAW.FACT_SALES
WHERE ORDER_ID > (SELECT COALESCE(MAX(ORDER_ID), 0) FROM ALFA_PROJEKT.GOLD.FACT_SALES);

-- 2. Inkrementální nahrání mezd (Payroll Fact)
INSERT INTO ALFA_PROJEKT.GOLD.FACT_PAYROLL (EMPLOYEE_ID, MONTHLY_SALARY, DATE_KEY)
SELECT 
    EMPLOYEE_ID, 
    MONTHLY_SALARY, 
    CAST(MONTH_YEAR AS DATE)
FROM ALFA_PROJEKT.RAW.FACT_PAYROLL
WHERE (EMPLOYEE_ID, CAST(MONTH_YEAR AS DATE)) NOT IN (SELECT EMPLOYEE_ID, DATE_KEY FROM ALFA_PROJEKT.GOLD.FACT_PAYROLL);

-- 3. Merge Dimenze Zaměstnanců
MERGE INTO ALFA_PROJEKT.GOLD.DIM_EMPLOYEES target
USING ALFA_PROJEKT.RAW.DIM_EMPLOYEES source
...
MERGE INTO ALFA_PROJEKT.GOLD.DIM_PRODUCTS target
USING ALFA_PROJEKT.RAW.DIM_PRODUCTS source
